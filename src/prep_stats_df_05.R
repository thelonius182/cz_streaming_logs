cz_licharod_stats_joined_04 <- read_rds(file = paste0(stats_data_flr(), "cz_stats_joined_04.RDS"))
...
n_sess_month_licharod <- cz_licharod_stats_report.1 %>% nrow()

# total listening hours this month
lihrs_month_licharod <- round(sum(cz_licharod_stats_report.1$cz_length) / 3600, 0)

# total number of devices this month
n_devices_month_licharod <-  cz_licharod_stats_report.1 %>% 
  group_by(cz_ipa, cz_dev_type) %>% 
  summarise(n_sess = n()) %>% 
  ungroup() %>% nrow()

# total reporting hours this month
report_hrs_licharod = round(int_length(interval(start = min(cz_licharod_stats_report.1$cz_ts_local),
                                       end = max(cz_licharod_stats_report.1$cz_ts_local))
) / 3600)


# listening hours 0: listeners and sessions by channel
n_lihrs_00 <- cz_licharod_stats_report.1 %>% 
  group_by(cha_name) %>% 
...
# devices by channel
n_lihrs_00a <- cz_licharod_stats_report.1 %>% 
...
n_lihrs_01 <- cz_licharod_stats_report.1 %>% 
...
n_lihrs_01a <- cz_licharod_stats_report.1 %>% 
  group_by(cha_name, cz_dev_type) %>% 
  summarise(n_sess = n()) %>% 
  ungroup(cz_dev_type) %>% 
...
n_lihrs_02 <- cz_licharod_stats_report.1 %>% 
...
n_lihrs_03 <- cz_licharod_stats_report.1 %>% 
...
cz_licharod_stats_report.2 <- n_lihrs_00 %>% 
  inner_join(n_lihrs_00a) %>% 
  inner_join(n_lihrs_01a) %>% 
  inner_join(n_lihrs_02) %>% 
  inner_join(n_lihrs_03) %>% 
  mutate(cz_lirod_stats_year = cz_year, cz_lirod_stats_month = cz_month) %>% 
...
cz_licharod_stats_report_total <- cz_licharod_stats_report.2 %>% 
  mutate(cz_lirod_stats_year = cz_year, cz_lirod_stats_month = cz_month) %>% 
  group_by(cz_lirod_stats_year, cz_lirod_stats_month) %>% 
...
cz_licharod_stats_report.4a <- cz_licharod_stats_report.1 %>% 
  group_by(hour_of_day) %>% 
  summarise(n_dev = n()) %>% 
  ungroup() %>% 
  mutate(cha_name = "TOTALEN")

cz_licharod_stats_report.4b <- cz_licharod_stats_report.1 %>% 
  select(cz_ipa, cz_dev_type, hour_of_day) %>% 
  distinct() %>% 
  group_by(hour_of_day) %>% 
  summarise(n_unique_dev = n())

cz_licharod_stats_report.4c <- cz_licharod_stats_report.4a %>% 
  inner_join(cz_licharod_stats_report.4b) %>% 
  mutate(pct_unique_dev = round(100 * n_unique_dev / n_dev)) %>% 
  select(cha_name, everything())

# listeners by channel by the hour of day ----
cz_licharod_stats_report.4d <- cz_licharod_stats_report.1 %>% 
  group_by(cha_name, hour_of_day) %>% 
  summarise(n_dev = n()) 

cz_licharod_stats_report.4e <- cz_licharod_stats_report.1 %>% 
  select(cz_ipa, cz_dev_type, cha_name, hour_of_day) %>% 
  distinct() %>% 
  group_by(cha_name, hour_of_day) %>% 
  summarise(n_unique_dev = n())

cz_licharod_stats_report.4f <- cz_licharod_stats_report.4d %>% 
  inner_join(cz_licharod_stats_report.4e) %>% 
  mutate(pct_unique_dev = round(100 * n_unique_dev / n_dev)) 

cz_licharod_stats_report.4g <- cz_licharod_stats_report.4f %>% bind_rows(cz_licharod_stats_report.4c)%>% 
  select(cha_name, hour_of_day, n_dev) %>% 
  pivot_wider(names_from = hour_of_day, names_prefix = "H", values_from = n_dev)

# by program ----
cz_licharod_stats_pgm_report.1 <- cz_licharod_stats_report.1 %>% 
...
cz_licharod_stats_pgm_partial.1b <- cz_licharod_stats_pgm_report.1 %>% 
...
cz_licharod_stats_pgm_partial.1b_rs <- cz_licharod_stats_pgm_partial.1b %>% 
  select(-pgm_title) %>% rowSums()  %>% as_tibble() %>% rename(hours_tot = value)

cz_licharod_stats_pgm_partial.1b1 <- cz_licharod_stats_pgm_partial.1b %>% bind_cols(cz_licharod_stats_pgm_partial.1b_rs)

# partial 2: unique listeners ----
cz_licharod_stats_pgm_partial.3a1 <- cz_licharod_stats_pgm_report.1 %>% 
...
cz_licharod_stats_pgm_partial.3a2 <- cz_licharod_stats_pgm_report.1 %>% 
  filter(is_abroad) %>% 
  group_by(pgm_title, cz_ipa, cz_dev_type) %>% 
  summarise(n1 = n()) %>% 
  ungroup(cz_ipa, cz_dev_type) %>% 
...
cz_licharod_stats_pgm_partial.3c <- cz_licharod_stats_pgm_partial.3a1 %>% 
  left_join(cz_licharod_stats_pgm_partial.3a2, by = "pgm_title") %>% 
...
cz_licharod_stats_pgm_partial.1a <- cz_licharod_stats_pgm_report.1 %>% 
...
cz_licharod_stats_pgm_partial.1a_rs <- cz_licharod_stats_pgm_partial.1a %>% 
  select(-pgm_title) %>% rowSums()  %>% as_tibble() %>% rename(sess_tot = value)

cz_licharod_stats_pgm_partial.1a1 <- cz_licharod_stats_pgm_partial.1a %>% bind_cols(cz_licharod_stats_pgm_partial.1a_rs)

# partial 4: coverage ----
cz_licharod_stats_pgm_partial.4a <- cz_licharod_stats_pgm_report.1 %>% 
...
# pgms assembled ----
cz_licharod_stats_pgm_report.2 <- cz_licharod_stats_pgm_partial.1b1 %>% 
  inner_join(cz_licharod_stats_pgm_partial.3c) %>% 
  inner_join(cz_licharod_stats_pgm_partial.1a1) %>% 
  inner_join(cz_licharod_stats_pgm_partial.4a) 

# cz_licharod_stats_pgms_report.tsv ----
write_rds(x = cz_licharod_stats_pgm_report.2,
          file = paste0(stats_data_flr(), "cz_licharod_stats_pgm_report.2.RDS"),
          compress = "gz")
