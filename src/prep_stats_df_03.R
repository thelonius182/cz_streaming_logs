...
# programs
# cz_programs.1 <- cz_stats_joined_03 %>% select(pgm_title) %>% distinct()
# 
# suppressPackageStartupMessages(library(snakecase))
# 
# cz_programs.2 <- cz_programs.1 %>% mutate(cz_title = str_trim(to_any_case(pgm_title, 
#                                                                           case = "title")))
# write_delim(x = cz_programs.2, file = "cz_pgm_titles.tsv", delim = "\t")

cz_pgm_titles_fixed.1 <- read_delim(
  "/home/lon/Downloads/cz_stats_pgm_titles - Blad1.tsv",
  delim = "\t",
  escape_double = FALSE,
  col_names = TRUE,
  trim_ws = TRUE
...
# names(cz_pgm_titles_fixed.1) <- c("pgm_title_key", "title_rmv", "pgm_title_fixed")
# 
# cz_pgm_titles_fixed.2 <- cz_pgm_titles_fixed.1 %>% 
#   select(-title_rmv)
...
rephrs_month = round(int_length(interval(start = min(cz_stats_report.1$cz_ts_local),
...
n_lihrs_00 <- cz_stats_report.1 %>% 
  ...
  summarise(lihrs = round(sum(cz_length) / 3600),
            n_sess = n()) %>% 
  mutate(mean_sess_per_hr = ceiling(n_sess / rephrs_month),
...
cz_year <- year(min(cz_stats_report.1$cz_ts_local))
cz_month <- month(min(cz_stats_report.1$cz_ts_local))
...
write_rds(x = cz_stats_report.3,
          file = "cz_stats_report.3.RDS",
          compress = "gz")

write_excel_csv2(x = cz_stats_report.3, file = "cz_stats_channel_report.tsv", delim = "\t")
...
write_rds(x = cz_stats_report.4g,
          file = "cz_stats_report.4g.RDS",
          compress = "gz")


write_excel_csv2(x = cz_stats_report.4g, file = "cz_stats_hours_report.tsv", delim = "\t")
...
write_rds(x = cz_stats_pgm_report.2,
          file = "cz_stats_pgm_report.2.RDS",
          compress = "gz")

write_excel_csv2(x = cz_stats_pgm_report.2, file = "cz_stats_pgms_report.tsv", delim = "\t")
